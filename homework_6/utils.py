...
def count_requests(filepath):
    with open(filepath, "r") as f:
        n = len(f.readlines())
    return n


def count_request_types(filepath):
    types = dict()
    with open(filepath, "r") as f:
        for line in f:
...
def most_frequent_requests(filepath):
...
def largest_4xx_requests(filepath):
    with open(filepath, 'r') as log:
...
def users_with_5xx_requests(filepath):
    with open(filepath, 'r') as log:
